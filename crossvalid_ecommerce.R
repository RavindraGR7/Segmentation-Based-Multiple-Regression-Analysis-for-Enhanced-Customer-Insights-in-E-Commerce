...
# Outlier removal functions (same as in your script)
outliers <- function(x) {
...
for (i in 1:num_clusters) {
  cluster_data <- subset(df_new, cluster == as.factor(i))
  model <- train(Yearly_Amount_Spent ~ Avg_Session_Length + Time_on_App + Length_of_Membership, data = cluster_data, method = "lm", trControl = control)
  models[[i]] <- model
}

# Summarize models (optional)
